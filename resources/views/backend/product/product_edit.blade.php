                            ...
                            <form method="post" action="{{ route('product.update') }}" id="myForm">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="form-group col-sm-10">
                                        <input name="name" class="form-control @error('name') is-invalid @enderror" type="text" value="{{ old('name', $product->name) }}">
                                        @error('name')
                                    ...
                                    <div class="col-sm-10">
                                        <select name="supplier_id" class="form-select @error('supplier_id') border-danger @enderror" aria-label="Default select example">
                                            ...
                                            @foreach ($supplier as $supp)
                                                <option value="{{ $supp->id }}" @selected(old('supplier_id', $product->supplier_id) == $supp->id) >{{ $supp->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('supplier_id')
                                            <div class="invalid-feedback">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <select name="unit_id" class="form-select @error('unit_id') border-danger @enderror" aria-label="Default select example">
                                            <option selected="">Open this select menu</option>
                                            @foreach ($unit as $uni)
                                                <option value="{{ $uni->id }}" @selected(old('unit_id', $product->unit_id) == $uni->id)>{{ $uni->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('unit_id')
                                            <div class="invalid-feedback">
